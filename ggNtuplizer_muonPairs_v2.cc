...
void ggNtuplizer::fillMuonsPairs(const edm::Event& e, const edm::EventSetup& es, math::XYZPoint& pv, reco::Vertex vtx) {
	...
	edm::View<pat::Muon>::const_iterator muindex_a;
	edm::View<pat::Muon>::const_iterator muindex_b;
	...
	if ((bitEleMuX == 0 || bitEleMuX == 1 || bitEleMuX == 2)){
		cout << "Pelo menos um dos Triggers disparou" << endl;
			...
			for (edm::View<pat::Muon>::const_iterator jMu = iMu; jMu != muonHandle->end(); ++jMu) {
				...
				if(tmpVertex.isValid()){
					...
					if( (ChiSquaredProbability(tmpVertex.totalChiSquared(), tmpVertex.degreesOfFreedom())) > 0.05f ){
						R_Iso1 = ( iMu->pfIsolationR04().sumChargedHadronPt + max( 0., iMu->pfIsolationR04().sumNeutralHadronEt + iMu->pfIsolationR04().sumPhotonEt - 0.5 * iMu->pfIsolationR04().sumPUPt ) ) /  (iMu->pt());
						R_Iso2 = ( jMu->pfIsolationR04().sumChargedHadronPt + max( 0., jMu->pfIsolationR04().sumNeutralHadronEt + jMu->pfIsolationR04().sumPhotonEt - 0.5 * jMu->pfIsolationR04().sumPUPt ) ) /  (jMu->pt());
						if ((R_Iso1 >= 0.35 && R_Iso2 >= 0.35 ))continue;         
						//          cout << "O valor de R_Iso1: " << R_Iso1 << "O valor de R_Iso2: " << R_Iso2 << endl;
						if(!((deltaR(iMu->eta(), iMu->phi(), jMu->eta(), jMu->phi()) < 0.3)))continue;
						//            cout << "-------- delta R menos que 0.3: " <<  endl;
						if(!((deltaR(iMu->eta(), iMu->phi(), jMu->eta(), jMu->phi()) > 0.02)))continue;//Para excluir leptons fantasmas
						if (!(((fabs(iMu->dB(pat::Muon::PV3D))/iMu->edB(pat::Muon::PV3D))< 4) && ((fabs(jMu->dB(pat::Muon::PV3D))/jMu->edB(pat::Muon::PV3D))< 4)))continue;//SIP < 4
						//            cout << "----------- O valor de SPI" << endl;
		...
		if(nGoodMuons > 0){
			for (edm::View<pat::Muon>::const_iterator iMu = muonHandle->begin(); iMu != muonHandle->end(); ++iMu) {
				const reco::TransientTrack &tta = transientTrackBuilder->build(muindex_a->bestTrack());

				for (edm::View<pat::Muon>::const_iterator jMu = iMu; jMu != muonHandle->end(); ++jMu) {
					const reco::TransientTrack &ttb = transientTrackBuilder->build(muindex_b->bestTrack());
					if (iMu == jMu) {  
						tmpMu4++;
						continue;
					}

					if( muindex_a == iMu || muindex_a == jMu || muindex_b == iMu || muindex_b == jMu )continue;
					...
					if(!(tmpVertex_Z.isValid()))continue;
					if(!((ChiSquaredProbability(tmpVertex_Z.totalChiSquared(), tmpVertex_Z.degreesOfFreedom())) > 0.05f ))continue;        
					//          cout << "Temos um vertice com probabilidade maior que 5 por cento" << endl;

					R_Iso3 = ( iMu->pfIsolationR04().sumChargedHadronPt + max( 0., iMu->pfIsolationR04().sumNeutralHadronEt + iMu->pfIsolationR04().sumPhotonEt - 0.5 * iMu->pfIsolationR04().sumPUPt ) ) /  (iMu->pt());
					R_Iso4 = ( jMu->pfIsolationR04().sumChargedHadronPt + max( 0., jMu->pfIsolationR04().sumNeutralHadronEt + jMu->pfIsolationR04().sumPhotonEt - 0.5 * jMu->pfIsolationR04().sumPUPt ) ) /  (jMu->pt());
					if ((R_Iso3 >= 0.35 && R_Iso4 >= 0.35 ))continue;
					//            cout << "O valor de R_Iso3: " << R_Iso3 << "O valor de R_Iso4: " << R_Iso4 << endl;
					if(!((deltaR(iMu->eta(), iMu->phi(), jMu->eta(), jMu->phi()) < 0.3)))continue;
					//              cout << "-------- delta R menos que 0.3: " <<  endl;
					if(!((deltaR(iMu->eta(), iMu->phi(), jMu->eta(), jMu->phi()) > 0.02)))continue;//Para excluir leptons fantasmas
					if (!(((fabs(iMu->dB(pat::Muon::PV3D))/iMu->edB(pat::Muon::PV3D))< 4) && ((fabs(jMu->dB(pat::Muon::PV3D))/jMu->edB(pat::Muon::PV3D))< 4)))continue;
					//            cout << "----------- O valor de SPI" << endl;
